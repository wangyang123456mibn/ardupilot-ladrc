...
# ===== LADRC v2: Multi-instance independent LESO states (ArduCopter) =====
...
# 4. AC_AttitudeControl.cpp - optional LADRC attitude compensation (ArduCopter specific)
patch_file("libraries/AC_AttitudeControl/AC_AttitudeControl.cpp", [
    ("_rate_bf_ff_sampler = AP::scheduler().get_perf_counter(AP_Scheduler::PERF_RATE_BF_FF);",
     "_rate_bf_ff_sampler = AP::scheduler().get_perf_counter(AP_Scheduler::PERF_RATE_BF_FF);\n    // [LADRC] Attitude controller ready for LADRC compensation\n    // Rate PID controllers (pitch/roll/yaw) use AC_PID with built-in LESO")
], optional=True)

# Note: AP_FW_Controller.cpp is NOT patched (fixed-wing only, not needed for ArduCopter)
print("  [SKIP] AP_FW_Controller.cpp (fixed-wing only, not needed for ArduCopter)")

print("LADRC patch v2 for ArduCopter complete!")
